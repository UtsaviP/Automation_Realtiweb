...
public class Basic extends AbstractComponent {
	...
	public void ResponsibleLawyer(String Responsible_Lawyer) throws InterruptedException {
	...
	public void FirmContactAddEditClear() {
		try {
			addFirmContact();
			editFirmContact();
			clearFirmContact();
	...
	public void File_Configuration() throws InterruptedException {
		switchToIframe(MiddlePortionFrame);
	...
	public void Transferee(String Single_Transferee, String Estate_Transferee, String Corporation_Transferee,
			Map<String, String> clientNames) throws InterruptedException {
		switchToIframe(MiddlePortionFrame);
		...
		for (String fieldName : new String[] { "Single_Transferee", "Estate_Transferee", "Corporation_Transferee" }) {
			...
			if (clientName != null) {
				...
				if (fieldName.equalsIgnoreCase("Single_Transferee")) {
					...
					// For Estate_Transferee, interact with the Estate checkbox and fill the input field									
					...
					// For Corporation_Transferee, select the appropriate value and fill the input field				
					Thread.sleep(2000);
					...
					if (isBirthdayFieldDisplayed) {
						Assert.assertTrue(false, "****Fail: BirthDate field shows even Corporation option selected*****");
					} else {
						System.out.println("****PASS: BirthDate field is not showing if Corporation option selected*****");
					}
				}
			}
		}
	...
	//Verify all Transferee(s) name on File navigation > Buyer side
		...
		Assert.assertTrue(isMatchFound_purchaser, "****Fail: Not All Buyer Names Found in File navigation >Buyer Side*****");
	}
	...
	public void Transferor(String Single_Transferor, String Estate_Transferor, String Corporation_Transferor,
		...
		// Assert based on the overall match flag
		Assert.assertTrue(isMatchFound_Seller, "****Fail: Not All Seller Names Found in File navigation >Seller Side*****");
	}

	
	// Verify Fire Insurance functionality working properly or not
	public void FireInsurance() throws InterruptedException {
	...
	public void MortgageBroker() throws InterruptedException {
		boolean isTestPassed = false;
	...
	//Verify Mortgage Broker ->Fee$ field and Pay from Trust functionality working proper or not
	public void mortgageBrokerFee(String mortgageBrokerFee) throws InterruptedException {
		try {
			...
			if (mortgageBrokerList.size() > 0) {
				...
				driver.findElement(By.xpath("//input[@id='mtgBrokerFee']")).click();
				driver.switchTo().defaultContent();

				driver.findElement(By.xpath("//div[@class='me-auto']//span[text()='Trust Statement']")).click();
				Thread.sleep(2000);
				switchToIframe(MiddlePortionFrame);

				String displayedMortgageBrokerFee = driver.findElement(By.xpath(
						"//span[text()='Mortgage Broker Fee']/following::td[1]/input[@datatype='money' and @subnodestatus='0'][1]"))
						.getAttribute("value");
				WebElement trustStatement = driver.findElement(By.xpath("//span[text()='Mortgage Broker Fee']"));

				boolean test = trustStatement.isDisplayed()
						&& formattedMortgageBrokerFee.equals(displayedMortgageBrokerFee);

				Assert.assertTrue(test, "PASS: Mortgage Broker Fee showing Trust Account Screen");
